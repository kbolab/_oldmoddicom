...
#' @useDynLib moddicom
#' @export
OLDRAD.NewMultiPIPOblique<-function(dataStorage, Structure, SeriesInstanceUID) {
  objService<-services()
...
#' Wrapper for C function
#' @useDynLib moddicom
OLDRAD.NewMultiPointInPolyObl<-function(totalX, totalY, nX, nY, associatedInstanceNumberVect,
                                     NumSlices, NumSlicesWithROI, 
                                     arrayInstanceNumber, arrayInstanceNumberWithROI, arrayPosizioneInstanceNumberWithROI,
                                     DICOMOrientationVectorWithROI, DICOMOrientationVector,minX,maxX,minY,maxY ) {
  # creates the PIPvector
